...
const Performance = () => {
  ...
  return (
    <>
      ...
      <div className="bg-white px-4 sm:px-6 md:px-8  py-8 md:py-10 lg:py-12 flex justify-center items-center min-h-screen">
        ...
        <div className="max-w-screen-xl mx-auto flex flex-col lg:flex-row items-center justify-center gap-y-8 md:gap-y-10 lg:gap-y-0 md:gap-x-16 lg:gap-x-20 h-full">
          ...
          <motion.div
            ...
            className="w-full lg:w-[500px] flex flex-col justify-center mt-12 md:mt-20 md:gap-8 lg:gap-10"
          >
            {performanceData.map((item) => (
              <div key={item.id} className="flex w-full items-start gap-4">
                <img
                  src={item.img}
                  alt={item.title}
                  className="w-12 md:w-14 lg:w-16 h-12 md:h-14 lg:h-16 rounded object-cover"
                />
                <div>
                  <h3 className="text-lg md:text-xl font-[jost] font-semibold text-black">
                    {item.title}
                  </h3>
                  <p className="text-gray-600 text-sm font-[Hind_Vadodara] md:text-base lg:text-base mt-1 line-clamp-3">
                    {item.description}
                  </p>
                </div>
              </div>
      ...
      {/* Stats Section */}
      <div className="bg-white px-4 sm:px-6 md:px-8 lg:px-8 py-8 lg:pb-20 md:py-10 lg:py-12  sm:pt-6 md:pt-8">
